import { useEffect, useState } from 'react'
import Loader from 'react-loaders'
import { MapContainer, TileLayer, Marker, Popup } from 'react-leaflet'
import { useRef } from 'react'
import emailjs from '@emailjs/browser'
import AnimatedLetters from '../AnimatedLetters'
import './index.scss'




const Contact = () => {
    const [letterClass, setLetterClass] = useState('text-animate')
    const bodyRef = useRef()
    const nameRef = useRef()
    const subjectRef = useRef()
    const emailRef = useRef()

  useEffect(() => {
    return setTimeout(() => {
      setLetterClass('text-animate-hover')
    }, 3000)
  }, []) 

  const sendEmail = (e) => {
    e.preventDefault()

    let templateParams = {
        to_name: nameRef.current.value,
        from_name: 'Gagan Sunar',
        message: bodyRef.current.value,
        reply_to: emailRef.current.value
    };


    emailjs
        .send(
            'service_156x3k9',
            'template_fxgr80i',
            templateParams,
            'AP4je5u_u1IGLi8hu'
        )
           .then(
                (response) => {
                    alert('Message successfully sent!')
                    window.location.reload(false)
                },
                (error) => {
                    console.log(error)
                    alert('Failed to send the message, please try again ')
                }
                
           )
 }
    return (
        <>
        <div className="container contact-page">
            <div className="text-zone">
                <h1>
                    <AnimatedLetters 
                    letterClass={letterClass}
                    strArray={['C', 'o', 'n', 't', 'a', 'c', 't', ' ', 'm', 'e']} 
                    idx={15}
                    />
                </h1>
                <p>
                    I am interested in freelance opportunities - especially ambitious or large projects, However, if you have other request or question, don't hesitate to contact me using below form either.
                </p>
                <div className="contact-form">
                <form onSubmit={sendEmail}>
                        <ul>
                            <li className="half">
                                <input type="text" name="name" placeholder="Name" ref={nameRef} required />
                            </li>
                            <li className="half">
                                <input type="email" name="email" placeholder="Email" ref={emailRef} required />
                            </li>
                            <li>
                                <input placeholder="Subject" type="text" name="subject" ref={subjectRef} required />
                            </li>
                            <li>
                                <textarea placeholder="Message" name="message" required ref={bodyRef}></textarea>
                            </li>
                            <li>
                                <input type="submit" className='flat-button' value="SEND" />
                            </li>
                        </ul>

                    </form>

                </div> 
            </div>
          <div className="info-map">
            Gagan Sunar,
            <br/>
            Nepal,
            <br />
            Surya Kot Marg, 44600<br />
            Koteshwor, Kathmandu <br />
            <span>gagaan.sunaar@gmail.com</span>
        </div>
        <div className="map-wrap">
            <MapContainer center={[27.672738, 85.345685]} zoom={13}>
                <TileLayer url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png" />
                <Marker position={[27.672738, 85.345685]}>
                    <Popup>Gagan lives here, come over for a cup of coffee :)</Popup>
                </Marker>
            </MapContainer>
        </div>
        </div>
            <Loader type="pacman" />
            </>
    )
}

export default Contact